...
static bool action_delete_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(data, action_delete_draw_top, res, "Impossibile eliminare il contenuto.");
    return true;
}
...
static void action_delete_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(deleteData->deleteInfo.finished) {
        ...
        if(R_SUCCEEDED(deleteData->deleteInfo.result)) {
            prompt_display_notify("Successo", "Eliminato.", COLOR_TEXT, NULL, NULL, NULL);
        }
...
static void action_delete_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Eliminazione", "Premi B per annullare.", true, data, action_delete_update, action_delete_draw_top);
        } else {
...
static void action_delete_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            prompt_display_yes_no("Conferma", loadingData->message, COLOR_TEXT, loadingData->deleteData, action_delete_draw_top, action_delete_onresponse);
        } else {
            error_display_res(NULL, NULL, loadingData->popData.result, "Impossibile popolare l'elenco dei contenuti.");
...
static void action_delete_internal(linked_list* items, list_item* selected, const char* message, bool recursive, bool includeBase, bool ciasOnly, bool ticketsOnly) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Impossibile allocare i dati di eliminazione.");
    ...
    if(R_FAILED(listRes)) {
        error_display_res(NULL, NULL, listRes, "Impossibile inizializzare il popolamento dell'elenco dei contenuti.");
    ...
    info_display("Caricamento", "Premi B per annullare.", false, loadingData, action_delete_loading_update, action_delete_loading_draw_top);
}

void action_delete_file(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "Eliminare il file selezionato?", false, true, false, false);
}

void action_delete_dir(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "Eliminare la cartella corrente?", true, true, false, false);
}

void action_delete_dir_contents(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "Eliminare tutto il contenuto della cartella corrente?", true, false, false, false);
}

void action_delete_dir_cias(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "Eliminare tutti i CIA nella cartella corrente?", false, false, true, false);
}

void action_delete_dir_tickets(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "Eliminare tutti i ticket nella cartella corrente?", false, false, false, true);
}