...
static void action_new_folder_onresponse(ui_view* view, void* data, SwkbdButton button, const char* response) {
    ...
    if(button == SWKBD_BUTTON_CONFIRM) {
        ...
        if(R_SUCCEEDED(res)) {
            ...
            prompt_display_notify("Successo", "Cartella creata.", COLOR_TEXT, NULL, NULL, NULL);
        } else {
            error_display_res(NULL, NULL, res, "Impossibile creare la cartella.");
        }
    }
...
void action_new_folder(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Impossibile allocare i dati della nuova cartella.");
    ...
    kbd_display("Inserisci il nome della cartella", "", SWKBD_TYPE_NORMAL, 0, SWKBD_NOTEMPTY_NOTBLANK, FILE_NAME_MAX, data, action_new_folder_onresponse);
}