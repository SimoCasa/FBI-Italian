...
static void action_extract_smdh_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(R_SUCCEEDED(res)) {
        prompt_display_notify("Successo", "SMDH estratto.", COLOR_TEXT, info, task_draw_title_info, NULL);
    } else {
        error_display_res(info, task_draw_title_info, res, "Impossibile estrarre SMDH.");
    }
}

static void action_extract_smdh_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        info_display("Estrazione SMDH", "", false, data, action_extract_smdh_update, task_draw_title_info);
    }
}

void action_extract_smdh(linked_list* items, list_item* selected) {
    prompt_display_yes_no("Conferma", "Estrarre l'SMDH del titolo selezionato?", COLOR_TEXT, selected->data, task_draw_title_info, action_extract_smdh_onresponse);
}