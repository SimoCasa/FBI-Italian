...
static void action_delete_title_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(R_FAILED(res)) {
        error_display_res(info, task_draw_title_info, res, "Impossibile eliminare il titolo.");
    } else {
        linked_list_remove(deleteData->items, deleteData->selected);
        task_free_title(deleteData->selected);

        prompt_display_notify("Successo", "Titolo eliminato.", COLOR_TEXT, NULL, NULL, NULL);
    }
...
static void action_delete_title_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        info_display("Eliminazione del titolo", "", false, data, action_delete_title_update, action_delete_title_draw_top);
    } else {
        free(data);
    }
}
...
static void action_delete_title_internal(linked_list* items, list_item* selected, const char* message, bool ticket) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Impossibile allocare i dati del titolo da eliminare.");
    ...
    prompt_display_yes_no("Conferma", message, COLOR_TEXT, data, action_delete_title_draw_top, action_delete_title_onresponse);
}

void action_delete_title(linked_list* items, list_item* selected) {
    action_delete_title_internal(items, selected, "Eliminare il titolo selezionato?", false);
}

void action_delete_title_ticket(linked_list* items, list_item* selected) {
    action_delete_title_internal(items, selected, "Eliminare il titolo e il ticket selezionati?", true);
}