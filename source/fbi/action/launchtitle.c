...
static void action_launch_title_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(R_FAILED(res)) {
        ...
        error_display_res(info, task_draw_title_info, res, "Impossibile avviare il titolo.");
    }
}

static void action_launch_title_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        info_display("Avvio del titolo", "", false, data, action_launch_title_update, task_draw_title_info);
    }
}

void action_launch_title(linked_list* items, list_item* selected) {
    prompt_display_yes_no("Conferma", "Avviare il seguente titolo?", COLOR_TEXT, selected->data, task_draw_title_info, action_launch_title_onresponse);
}