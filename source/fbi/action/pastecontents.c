...
static bool action_paste_contents_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(data, action_paste_contents_draw_top, res, "Impossibile incollare il contenuto.");
    return true;
}
...
static void action_paste_contents_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(pasteData->pasteInfo.finished) {
        ...
        if(R_SUCCEEDED(pasteData->pasteInfo.result)) {
            prompt_display_notify("Successo", "Contenuto incollato.", COLOR_TEXT, NULL, NULL, NULL);
        }
    ...
    *progress = pasteData->pasteInfo.currTotal != 0 ? (float) ((double) pasteData->pasteInfo.currProcessed / (double) pasteData->pasteInfo.currTotal) : 0;
    snprintf(text, PROGRESS_TEXT_MAX, "%lu / %lu\n%.2f %s / %.2f %s\n%.2f %s/s, Tempo %s", pasteData->pasteInfo.processed, pasteData->pasteInfo.total,
...
static void action_paste_contents_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Incollamento del contenuto", "Premi B per annullare.", true, data, action_paste_contents_update, action_paste_contents_draw_top);
        } else {
            error_display_res(NULL, NULL, res, "Impossibile avviare l'operazione incolla.");
...
static void action_paste_contents_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            prompt_display_yes_no("Conferma", "Incollare il contenuto degli appunti nella directory corrente?", COLOR_TEXT, loadingData->pasteData, action_paste_contents_draw_top, action_paste_contents_onresponse);
        } else {
            error_display_res(NULL, NULL, loadingData->popData.result, "Impossibile popolare l'elenco dei contenuti degli appunti.");
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "Recupero dell'elenco dei contenuti degli appunti...");
}

void action_paste_contents(linked_list* items, list_item* selected) {
    if(!clipboard_has_contents()) {
        prompt_display_notify("Fallimento", "Appunti vuoti.", COLOR_TEXT, NULL, NULL, NULL);
        return;
    }

    paste_contents_data* data = (paste_contents_data*) calloc(1, sizeof(paste_contents_data));
    if(data == NULL) {
        error_display(NULL, NULL, "Impossibile allocare i dati del contenuto incollato.");
    ...
    if(R_FAILED(listRes)) {
        error_display_res(NULL, NULL, listRes, "Impossibile avviare il popolamento dell'elenco dei contenuti degli appunti.");
    ...
    info_display("Caricamento", "Premi B per annullare.", false, loadingData, action_paste_contents_loading_update, action_paste_contents_loading_draw_top);
}