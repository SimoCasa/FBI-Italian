...
static void action_delete_ext_save_data_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(R_FAILED(res)) {
        error_display_res(info, task_draw_ext_save_data_info, res, "Impossibile eliminare i salvataggi ext.");
    } else {
        linked_list_remove(deleteData->items, deleteData->selected);
        task_free_ext_save_data(deleteData->selected);

        prompt_display_notify("Success", "Salvataggi ext eliminati.", COLOR_TEXT, NULL, NULL, NULL);
    }
...
static void action_delete_ext_save_data_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        info_display("Eliminazione Salvataggi Ext", "", false, data, action_delete_ext_save_data_update, action_delete_ext_save_data_draw_top);
    } else {
        free(data);
    }
}
...
void action_delete_ext_save_data(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Impossibile allocare i salvataggi ext eliminati.");
    ...
    prompt_display_yes_no("Conferma", "Eliminare i salvataggi ext selezionati?", COLOR_TEXT, data, action_delete_ext_save_data_draw_top, action_delete_ext_save_data_onresponse);
}