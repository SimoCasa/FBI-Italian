...
static bool action_delete_tickets_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(data, action_delete_tickets_draw_top, res, "Impossibile eliminare ticket(s).");
    return true;
}
...
static void action_delete_tickets_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(deleteData->deleteInfo.finished) {
        ...
        if(R_SUCCEEDED(deleteData->deleteInfo.result)) {
            prompt_display_notify("Successo", "Ticket(s) eliminato/i.", COLOR_TEXT, NULL, NULL, NULL);
        }
...
static void action_delete_tickets_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Eliminazione", "Premi B per annullare.", true, data, action_delete_tickets_update, action_delete_tickets_draw_top);
        } else {
...
static void action_delete_tickets_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            prompt_display_yes_no("Conferma", loadingData->message, COLOR_TEXT, loadingData->deleteData, action_delete_tickets_draw_top, action_delete_tickets_onresponse);
        } else {
...
static void action_delete_tickets_internal(linked_list* items, list_item* selected, const char* message, bool unused) {
    ...
    if(unused) {
        ...
        info_display("Caricamento", "Premi B per annullare.", false, loadingData, action_delete_tickets_loading_update, action_delete_tickets_loading_draw_top);
    } else {
        ...
        prompt_display_yes_no("Conferma", message, COLOR_TEXT, data, action_delete_tickets_draw_top, action_delete_tickets_onresponse);
    }
}

void action_delete_ticket(linked_list* items, list_item* selected) {
    action_delete_tickets_internal(items, selected, "Eliminare il ticket selezionato?", false);
}

void action_delete_tickets_unused(linked_list* items, list_item* selected) {
    action_delete_tickets_internal(items, selected, "Eliminare tutti i tickets non utilizzati?", true);
}
