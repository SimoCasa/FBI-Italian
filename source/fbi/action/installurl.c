    ...
    if(*(u16*) initialReadBlock == 0x2020) {
        ...
        if(R_SUCCEEDED(res = cia_get_title_id(&titleId, (u8*) initialReadBlock, installData->installInfo.bufferSize))) {
            ...
            if(R_SUCCEEDED(APT_CheckNew3DS(&n3ds)) && !n3ds && ((titleId >> 28) & 0xF) == 2) {
                ui_view* view = prompt_display_yes_no("Conferma", "Il titolo è destinato ai sistemi New 3DS.\nContinuare?", COLOR_TEXT, data, action_install_url_draw_top, action_install_url_n3ds_onresponse);
                if(view != NULL) {
...
static bool action_install_url_error(void* data, u32 index, Result res, ui_view** errorView) {
    ...
    if(strlen(url) > 38) {
        *errorView = error_display_res(data, action_install_url_draw_top, res, "Impossibile installare dall'URL.\n%.35s...", url);
    } else {
        *errorView = error_display_res(data, action_install_url_draw_top, res, "Impossibile installare dall'URL.\n%.38s", url);
    }
...
static void action_install_url_install_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    *progress = installData->installInfo.currTotal != 0 ? (float) ((double) installData->installInfo.currProcessed / (double) installData->installInfo.currTotal) : 0;
    snprintf(text, PROGRESS_TEXT_MAX, "%lu / %lu\n%.2f %s / %.2f %s\n%.2f %s/s, Tempo %s", installData->installInfo.processed, installData->installInfo.total,
...
static void action_install_url_confirm_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Installazione dall'URL(s)", "Premi B per annullare.", true, data, action_install_url_install_update, action_install_url_draw_top);
        } else {
            error_display_res(NULL, NULL, res, "Impossibile avviare l'installazione.");
...
void action_install_url(const char* confirmMessage, const char* urls, const char* paths, void* userData,
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Impossibile allocare i dati di installazione dell'URL.");
    ...
    prompt_display_yes_no("Conferma", confirmMessage, COLOR_TEXT, data, action_install_url_draw_top, action_install_url_confirm_onresponse);
}