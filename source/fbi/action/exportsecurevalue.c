...
static void action_export_secure_value_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(R_SUCCEEDED(res = FSUSER_GetSaveDataSecureValue(&exists, &value, SECUREVALUE_SLOT_SD, (u32) ((info->titleId >> 8) & 0xFFFFF), (u8) (info->titleId & 0xFF)))) {
        if(!exists) {
            ...
            prompt_display_notify("Fallimento", "Secure value non impostata.", COLOR_TEXT, info, task_draw_title_info, NULL);
    ...
    if(R_SUCCEEDED(res)) {
        prompt_display_notify("Successo", "Secure value esportata.", COLOR_TEXT, info, task_draw_title_info, NULL);
    } else {
        error_display_res(info, task_draw_title_info, res, "Impossibile esportare secure value.");
    }
}

static void action_export_secure_value_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        info_display("Esportazione Secure Value", "", false, data, action_export_secure_value_update, task_draw_title_info);
    }
}

void action_export_secure_value(linked_list* items, list_item* selected) {
    prompt_display_yes_no("Conferma", "Esportare la secure value del titolo selezionato?", COLOR_TEXT, selected->data, task_draw_title_info, action_export_secure_value_onresponse);
}