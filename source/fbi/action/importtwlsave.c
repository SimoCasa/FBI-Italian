...
static bool action_import_twl_save_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(((import_twl_save_data*) data)->title, task_draw_title_info, res, "Impossibile importare il salvataggio.");
    return true;
}
...
static void action_import_twl_save_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(importData->importInfo.finished) {
        ...
        if(R_SUCCEEDED(importData->importInfo.result)) {
            prompt_display_notify("Successo", "Salvataggio importato.", COLOR_TEXT, importData->title, task_draw_title_info, NULL);
        }
...
static void action_import_twl_save_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Importazione Salvataggio", "Premi B per annullare.", true, data, action_import_twl_save_update, action_import_twl_save_draw_top);
        } else {
            error_display_res(importData->title, task_draw_title_info, res, "Impossibile avviare l'importazione del salvataggio.");
            free(data);
        }
    } else {
        free(data);
...
void action_import_twl_save(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Impossibile allocare i dati di salvataggio TWL di importazione.");
    ...
    prompt_display_yes_no("Conferma", "Importare il salvataggio del titolo selezionato?", COLOR_TEXT, data, action_import_twl_save_draw_top, action_import_twl_save_onresponse);
}