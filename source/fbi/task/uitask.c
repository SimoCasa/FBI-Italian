...
void task_draw_file_info(ui_view* view, void* data, float x1, float y1, float x2, float y2) {
    ...
    if(strlen(info->name) > 48) {
        infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Nome: %.45s...\n", info->name);
    } else {
        infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Nome: %.48s\n", info->name);
    }

    infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Attributi: ");
    ...
    if(info->attributes & (FS_ATTRIBUTE_DIRECTORY | FS_ATTRIBUTE_HIDDEN | FS_ATTRIBUTE_ARCHIVE | FS_ATTRIBUTE_READ_ONLY)) {
        ...
        if(info->attributes & FS_ATTRIBUTE_DIRECTORY) {
            infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Cartella");
            needsSeparator = true;
        }
        ...
        if(info->attributes & FS_ATTRIBUTE_HIDDEN) {
            ...
            infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Nascosto");
            needsSeparator = true;
        }
        ...
        if(info->attributes & FS_ATTRIBUTE_ARCHIVE) {
            ...
            infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Archivio");
            needsSeparator = true;
        }
        ...
        if(info->attributes & FS_ATTRIBUTE_READ_ONLY) {
            ...
            infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Sola Lettura");
            needsSeparator = true;
        }
    } else {
        infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Nessuno");
    }

    infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "\n");

    if(!(info->attributes & FS_ATTRIBUTE_DIRECTORY)) {
        infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Dimensione: %.2f %s\n",
        ...
        if(info->isCia && info->ciaInfo.loaded) {
            ...
            if(info->ciaInfo.hasMeta) {
                ...
                snprintf(regionString, sizeof(regionString), "Sconosciuto");
            }
...
void task_draw_pending_title_info(ui_view* view, void* data, float x1, float y1, float x2, float y2) {
    ...
    snprintf(infoText, sizeof(infoText),
             "Title ID in sospeso: %016llX\n"
                     "Media Type: %s\n"
                     "Versione: %hu (%d.%d.%d)",
             info->titleId,
             info->mediaType == MEDIATYPE_NAND ? "NAND" : info->mediaType == MEDIATYPE_SD ? "SD" : "Cartuccia",
...
void task_draw_system_save_data_info(ui_view* view, void* data, float x1, float y1, float x2, float y2) {
    ...
    snprintf(infoText, sizeof(infoText), "ID salvataggio di sistema: %08lX", info->systemSaveDataId);
